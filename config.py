...
def _env_as_float(name: str) -> float:
    ...
    try:
        ...
        raise RuntimeError(f"Environment variable '{name}' must be a float, got '{value}'.") from exc
...
def _env_as_int(name: str) -> int:
    ...
    try:
        ...
        raise RuntimeError(f"Environment variable '{name}' must be an int, got '{value}'.") from exc
